...
void CProjectile::Tick()
{
	...
	int Collide = GS()->Collision(GetMapID())->IntersectLine(PrevPos, CurPos, &CurPos, 0);
	CCharacter *OwnerChar = GS()->GetPlayerChar(m_Owner);
	CCharacter *TargetChr = GS()->m_World.IntersectCharacter(PrevPos, CurPos, 6.0f, CurPos, GetMapID(), OwnerChar);
	...
	if(TargetChr || Collide || m_LifeSpan < 0 || GameLayerClipped(CurPos))
	{
		if(m_LifeSpan >= 0 || m_Weapon == WEAPON_GRENADE)
			GS()->CreateSound(CurPos, m_SoundImpact, -1, GetMapID());

		if(m_Explosive)
			GS()->CreateExplosion(CurPos, m_Owner, m_Weapon, false, GetMapID());
