...
{
	return round_to_int(CheckPos.x)/32 < -200 || round_to_int(CheckPos.x)/32 > GS()->Collision(GetMapID())->GetWidth()+200 ||
			round_to_int(CheckPos.y)/32 < -200 || round_to_int(CheckPos.y)/32 > GS()->Collision(GetMapID())->GetHeight()+200 ? true : false;
}
