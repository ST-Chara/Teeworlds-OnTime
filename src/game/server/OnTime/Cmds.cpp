...
bool CCmd::ChatCmd(CNetMsg_Cl_Say *Msg)
{
    if(!strncmp(Msg->m_pMessage, "/login", 6))
	{
		...
		{
            GS()->SendChatTarget(m_pPlayer->GetCID(), _("Use: /login <username> <password>"));
            return true;
        }

		if(str_length(Username) > 15 || str_length(Username) < 4 || str_length(Password) > 15 || str_length(Password) < 4) 
        {
            GS()->SendChatTarget(m_pPlayer->GetCID(), _("Username / Password must contain 4-15 characters"));
            return true;
        }
	...
	else if(!strncmp(Msg->m_pMessage, "/register", 9))
	{
		...
		{	
            GS()->SendChatTarget(m_pPlayer->GetCID(), _("Use /register <username> <password>'"));
            return true;
        }
		if(str_length(Username) > 15 || str_length(Username) < 4 || str_length(Password) > 15 || str_length(Password) < 4)
		{
            GS()->SendChatTarget(m_pPlayer->GetCID(), _("Username and Password must contain 4-15 characters"));
            return true;
        }
        else if (!strcmp(Username, Password))
		{	
            GS()->SendChatTarget(m_pPlayer->GetCID(), _("Username and Password must be different"));
            return true;
        }
...
void CCmd::LastChat()
{
	 m_pPlayer->m_LastChat = GS()->Server()->Tick();
}
